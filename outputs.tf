...
output "ssh_command" {
  ...
  value       = "ssh -i tools/hermes_lab_key -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${cloudstack_ipaddress.lab.ip_address}"
}
...
output "credentials" {
  ...
  value = {
    ...
    ssh     = "ssh -i tools/hermes_lab_key -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${cloudstack_ipaddress.lab.ip_address}"
  }
}
